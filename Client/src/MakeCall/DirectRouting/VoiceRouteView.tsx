 ...
 */
const validateVoiceRoutes = (voiceRoutes: VoiceRoute[]): VoiceRoute[] =>
  voiceRoutes.map((route) => ({ ...route, errors: validateVoiceRoute(route) }));
 ...
 * @returns View
 */
const VoiceRouteView: React.FC<VoiceRouteViewProps> = ({
  voiceRoute,
  trunks,
  onChange,
  disabled,
  ...
  >
    <div className="d-flex mb-1 center-content">
      <div className="ms-Grid-row f-1 mr-3">
        ...
        <div className="ms-Grid-col ms-lg4">
          <TrunkDropdown voiceRoute={voiceRoute} trunks={trunks} onChange={onChange} disabled={disabled} />
        </div>
      </div>
      {onDelete != null && (
        <PrimaryButton
...
type TrunkDropdownProps = {
  voiceRoute: VoiceRoute;
  trunks: Trunk[];
} & Omit<IDropdownProps, 'options' | 'onChange'> &
  Pick<VoiceRouteViewProps, 'onChange'>;
...
const TrunkDropdown: React.FC<TrunkDropdownProps> = ({ voiceRoute, trunks, onChange: propsOnChange, ...props }) => {
  const [selectedTrunks, setSelectedTrunks] = React.useState<Trunk[]>(() =>
    initializeSelectedTrunks(voiceRoute, trunks)
  );
      ...
      if (option.selected) {
        ...
        setSelectedTrunks((oldSelectedTrunks) => {
          ...
          propsOnChange(
            'trunks',
            voiceRoute,
            newSelectedTrunks.map(({ fqdn }) => fqdn)
          );
  ...
  React.useEffect(() => {
    ...
    setSelectedTrunks((oldTrunks) => trunks.filter((trunk) => oldTrunks.some((oldTrunk) => oldTrunk.key == trunk.key)));
  }, [trunks]);
