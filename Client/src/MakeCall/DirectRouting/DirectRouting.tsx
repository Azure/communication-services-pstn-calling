...
import TrunkView, { emptyTrunk, Trunk, TrunkViewHeader, validateTrunks } from './TrunkView';
import VoiceRouteView, { emptyVoiceRoute, validateVoiceRoutes, VoiceRoute, VoiceRoutesHeader } from './VoiceRouteView';
...
const DirectRouting: React.FC<DirectRoutingPropsType> = ({ disabled: propsDisabled, className, style }) => {
  ...
  const [voiceRoutes, setVoiceRoutes] = React.useState<VoiceRoute[]>(
    [0, 1, 2].map((key) => ({ ...emptyVoiceRoute, key }))
  );
  ...
  const nonEmptyVoiceRoutes = voiceRoutes.filter(
    (route) =>
  ...
  const isVoicesRoutesFaulty =
    nonEmptyVoiceRoutes.filter(({ errors }) => errors && Object.keys(errors).length > 0).length > 0;
   ...
   * @returns void
   */
  const onTrunkChange = (field: 'fqdn' | 'sipSignalingPort', trunk: Trunk, newValue: string) =>
    setTrunks((oldList) =>
      validateTrunks([
   ...
   * @returns void
   */
  const onVoiceRouteChange = (field: string, voiceRoute: VoiceRoute, newValue: string | string[]) =>
  ...
  const onTrunkDelete = (key) => () => {
    ...
    setVoiceRoutes((oldRoutes) => {
      let newRoutes = [
        ...
        ...oldRoutes
          .filter((route) => route.trunks != null && route.trunks.includes(trunk.fqdn))
          .map((route) => ({
            ...route,
  ...
  const onCreateClick = async () => {
    ...
    const routes: SipTrunkRoute[] = nonEmptyVoiceRoutes.map(({ name, numberPattern, trunks }) => ({
      name,
      numberPattern,
      trunks
    }));
    ...
    <div className={className} style={style}>
      <div className="ms-Grid-row mt-3">
        <div className="call-input-panel ms-Grid-col ms-sm12 ms-lg12 ms-xl12 ms-xxl6">
          ...
          <div className="mb-3">
            Direct routing allows your Session Border Controllers (Trunks) to make calls through Azure Communication
            Services. Get started by adding your supported Session Border Controller (Trunk).
          </div>
        ...
        <div className="call-input-panel ms-Grid-col ms-sm12 ms-lg12 ms-xl12 ms-xxl6">
          ...
          <div className="mb-3">
            Azure Communication Services allows balancing and routing outgoing calls based on called number. Set voice
            routes to complete your direct routing configuration. Use the move up and move down buttons to set the
            priority of voice routes.
          </div>
      ...
      >
        <DialogFooter>
          <PrimaryButton className="primary-button" onClick={() => setShowUploadedDialog(false)}>
            Close
          </PrimaryButton>
        </DialogFooter>
      </Dialog>
