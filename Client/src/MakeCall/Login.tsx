...
const Login: React.FC<LoginProps> = ({ onLoggedIn }) => {
    ...
    const provisionNewUser = async (): Promise<void> => {
        try {
            ...
            const client = await new CommunicationIdentityClient(connectionString)
    ...
    return (
        <Card 
            ...
            <div>The example code shows how to use the ACS Administration SDK from a backend service. A walkthrough of integrating the ACS Administration SDK can be found on 
                <a className="sdk-docs-link" rel="noopener noreferrer" target="_blank" href="https://docs.microsoft.com/en-us/azure/communication-services/quickstarts/access-tokens?pivots=programming-language-javascript">Microsoft Docs</a></div>
            {
                isLoggedIn && 
                <div>
                    <br></br>
