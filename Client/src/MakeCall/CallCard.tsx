...
import { Features, LocalAudioStream, Call, DeviceManager, CallState, RemoteParticipant, AudioDeviceInfo } from '@azure/communication-calling';
...
const CallCard: React.FC<CallCardPropType> = ({call, deviceManager, speakerDeviceOptions: propsSpeakerDeviceOptions, microphoneDeviceOptions: propsMicrophoneDeviceOptions, selectedSpeakerDeviceId: propsSelectedSpeakerDeviceId, selectedMicrophoneDeviceId: propsSelectedMicrophoneDeviceId}) => {
    const [callState, setCallState] = React.useState<CallState>(call.state);
    const [callId, setCallId] = React.useState<string>(call.id);
    const [remoteParticipants, setRemoteParticipants] = React.useState<readonly RemoteParticipant[]>(call.remoteParticipants);
    ...
    const [speakerDeviceOptions, setSpeakerDeviceOptions] = React.useState<AudioDeviceInfo[]>(propsSpeakerDeviceOptions != null ? propsSpeakerDeviceOptions : []);
    const [microphoneDeviceOptions, setMicrophoneDeviceOptions] = React.useState<AudioDeviceInfo[]>(propsMicrophoneDeviceOptions != null ? propsMicrophoneDeviceOptions : []);
    const [selectedMicrophoneDeviceId, setSelectedMicrophoneDeviceId] = React.useState<string>(propsSelectedMicrophoneDeviceId)
    ...
    const [dominantRemoteParticipant, setDominantRemoteParticipant] = React.useState<RemoteParticipant | undefined>(undefined);
    ...
    const speakerDeviceSelectionChanged = async (event: React.FormEvent<HTMLDivElement>, option?: IDropdownOption): Promise<void> => {
        if (option == null) {
            return;
        }
    ...
    const microphoneDeviceSelectionChanged = async (event: React.FormEvent<HTMLDivElement>, option?: IDropdownOption): Promise<void> => {
        if (option == null) {
            return;
        }
        ...
        if (call != null) {
            ...
            call.on('remoteParticipantsUpdated', event => {
                ...
                event.removed.forEach(p => {
                    ...
                    if(p.callEndReason) {
                        setCallMessage(prevMessage => `${prevMessage ? prevMessage + `\n` : ``}
            ...
            const dominantSpeakersChangedHandler = async () => {
                try {
                    if(isDominantSpeakerMode) {
                        ...
                        const newDominantSpeakerIdentifier = call.feature(Features.DominantSpeakers).dominantSpeakers.speakersList[0];
                        if (newDominantSpeakerIdentifier) {
                            ...
                            const newDominantRemoteParticipant = utils.getRemoteParticipantObjFromIdentifier(call, newDominantSpeakerIdentifier);
    ...
    return (
        <div className="ms-Grid mt-2">
            ...
            <div className="ms-Grid-row">
                    ...
                    <div className="ms-Grid-col ms-sm12 ms-lg12 ms-xl12 ms-xxl3">
                        <div className="participants-panel mt-1 mb-3">
                            <Toggle label={
                                ...
                                onText="On"
                                offText="Off"
                                onChange={() => { toggleDominantSpeakerMode()}}
                            />
                            {
                                isDominantSpeakerMode &&
                                <div>
                                    Current dominant speaker: {dominantRemoteParticipant ? utils.getIdentifierText(dominantRemoteParticipant.identifier) : `None`}
                                </div>
                            }
                            ...
                            <ul className="participants-panel-list">
                                {
                                    remoteParticipants.map(remoteParticipant =>
                                        <RemoteParticipantCard key={`${utils.getIdentifierText(remoteParticipant.identifier)}`} remoteParticipant={remoteParticipant} call={call} />
                                    )
                                }
                            </ul>
                        </div>
                ...
                <div className={callState === 'Connected' ? `ms-Grid-col ms-sm12 ms-lg12 ms-xl12 ms-xxl9` : 'ms-Grid-col ms-sm12 ms-lg12 ms-xl12 ms-xxl12'}>
                    <div className="mb-2">
                        {
                            callState !== 'Connected' &&
                        ...
                        <div className="text-center">
                            ...
                            </span>
                            {
                                ...
                                <span className="in-call-button"
                                    ...
                                    {
                                        (callState === 'Connected' || callState === 'RemoteHold') &&
                                        <Icon iconName="Play" />
                                    }
                                </span>
                            }
                            ...
                            <span className="in-call-button"
                                title={`${isOutgoingAudioMediaAccessActive ? 'Clear audio effect' : 'Apply outgoing audio effect'} to call`}
                                // variant="secondary"
                                onClick={() => handleOutgoingAudioEffect()}>
                                {
                                    isOutgoingAudioMediaAccessActive &&
                                    ...
                                    <div className="pl-2">
                                        {
                                            ...
                                            <Dropdown
                                                ...
                                                label={'Speaker'}
                                                placeHolder={speakerDeviceOptions.length === 0 ? 'No speaker devices found' : selectedSpeakerDeviceId}
                                        ...
                                        }
                                        {
                                            ...
                                            <Dropdown
                                                ...
                                                label={'Microphone'}
                                                placeHolder={microphoneDeviceOptions.length === 0 ? 'No microphone devices found' : selectedMicrophoneDeviceId}
                                                styles={{ dropdown: { width: 400 } }}
                                            />
                                        }
                                    </div>
