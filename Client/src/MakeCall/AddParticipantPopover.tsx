...
const AddParticipantPopover: React.FC<AddParticipantPopoverProps> = ({ call }) => {
  ...
  return (
    <>
      ...
      <span>
        <a href="#" onClick={toggleAddParticipantPanel}>
          <i className="add-participant-button ms-Icon ms-Icon--AddFriend" aria-hidden="true"></i>
        </a>
      </span>
              ...
              <div className="add-participant-panel">
                ...
                <div className="add-participant-panel-header">
                  <TextField
                    className="text-left"
                    label="Identifier"
                    onChange={(event) => setUserId((event.target as HTMLTextAreaElement).value)}
                  />
                  <TextField
                    className="text-left"
                    label="Alternate Caller Id (For adding phone number only)"
                    onChange={(event) => setAlternateCallerId((event.target as HTMLTextAreaElement).value)}
                  />
                  <PrimaryButton className="mt-3" onClick={handleAddCommunicationUser}>
                    Add CommunicationUser
                  </PrimaryButton>
                  <PrimaryButton className="mt-1" onClick={handleAddPhoneNumber}>
                    Add Phone Number
                  </PrimaryButton>
                </div>
              </div>
